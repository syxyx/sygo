...
### 7/19 全站图片性能优化
- **问题**：22 张图片共 3.5MB，首页 9 张校区图 1.6MB，手机加载 8-12 秒
- **根因**：1200px 宽图片在 400px 卡片中显示，浪费 3 倍带宽；宿舍图片无懒加载
- **方案**：sharp 批量创建 600px WebP 卡片版（`-card.webp`），`<picture>` 元素卡片用小图 + lightbox 用原图，宿舍图加 `loading="lazy"`
- **关键发现**：纯 WebP 格式转换只省 12%，真正瓶颈在尺寸——缩小到 600px + WebP 省 74%（3326KB→851KB）
- **效果**：首屏图片从 1.6MB → 34KB，其余滚动才加载
...
- **图片优化先缩尺寸再换格式**：纯 WebP 转换只省 ~12%，先缩小尺寸再转 WebP 能省 74%。用 `<picture>` 区分卡片/灯箱图片，sharp 批量处理
...
- [x] **全站图片性能优化**（7/19，WebP卡片版+懒加载，首屏1.6MB→34KB）
- [x] 压缩首页校区图片（已通过 600px WebP 卡片版解决）
