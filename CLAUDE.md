...
- [x] **留言墙：公开回复（网页内，暗号+密码）**（2026-07-10 已配置+实测通过）— 留言框打暗号「回复」→ 弹密码框 → Supabase Auth 登录(`owner@sygo.top`)→ 解锁回复模式，展开列表点「回复」直接写。安全靠 RLS：只有登录站长(`auth.uid()` 非空)能 update，访客改不了。Supabase 侧已建 owner@sygo.top 账号 + 跑了「加 reply 字段 + `owner update` 策略」的 SQL。前端 `MessageWall.jsx`+数据层 `supabase.js`
